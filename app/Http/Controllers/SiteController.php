    ...
    public function index() {
        try {
            $products = Product::where('products.is_active', 1)
                ...
                ->groupByRaw('`products`.`id`, `products`.`title`, `products`.`description`, `products`.`image_url`, `products`.`price`, `products`.`is_active`, `products`.`is_featured`, `products`.`period`, `products`.`duration`, `products`.`iType`, `products`.`allowed_traffic`, `products`.`maximum_connections`, `products`.`created_at`, `products`.`updated_at`, `products`.`deleted_at`')
                ->get();
